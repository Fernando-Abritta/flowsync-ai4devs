...
_Motivo de la propuesta: el requisito de origen pide que sea «evidente sin cálculo mental», y eso no se puede suspender en una prueba. Esta versión exige una señal propia, no solo mostrar la fecha._
...
_Motivo de la propuesta: el PRD no dice si se permite. Se propone permitirlo, porque bloquearlo añadiría una regla de las que este producto rechaza y hay un caso legítimo evidente: anotar algo que ya debería estar hecho._
...
_Motivo de la propuesta: hace falta fijar la frontera. Borrar una tarea sí confirma porque es irreversible; quitar una fecha se deshace en un gesto, y confirmarlo sería fricción gratuita._
...
_Motivo de la propuesta: se deduce de los roles planos, pero el PRD nunca lo dice de la fecha en concreto, y es justo el campo donde alguien esperaría una excepción._
...
_Motivo de la propuesta: se deduce de la regla, pero conviene dejarlo escrito porque separa dos comportamientos muy distintos — que el vencimiento se decida en el momento de mirar, o que se congele en el momento de guardar. Solo el primero es compatible con CA-19._
...
Cada ticket es una unidad de trabajo que una persona termina en una sesión, con media jornada como techo. **Heredan los criterios de arriba**: su Definition of Done es una checklist de _cómo entregamos_, no criterios nuevos. Nombran la capa que tocan; **no diseñan el esquema** (tipo de columna, nulabilidad, índices, rutas y códigos de estado se deciden al implementar).
...
- [ ] Se implementa como una migración SQL versionada de Flyway; no se edita una migración que ya haya sido aplicada.
- [ ] Las tareas anteriores a la migración siguen siendo válidas después de aplicarla.
- [ ] El mapeo JPA valida contra el esquema administrado por Flyway con `ddl-auto=validate`; Hibernate no crea ni altera tablas.
- [ ] `./mvnw verify` deja pruebas y Spotless en verde.

**Depende de:** que la tarea exista (historia de creación, fuera de FS-118).
**Nota de riesgo:** Flyway es acumulativo y no usa un reverso automático. La prueba debe partir de H2 en memoria o de una base desechable; nunca debe modificar el archivo H2 de desarrollo.
...
- [ ] La respuesta se construye con un DTO; no se serializa la entidad JPA en crudo.
- [ ] La entrada se valida con Jakarta Validation y una fecha no válida se rechaza señalando el campo, con el mismo formato de error que ya usa el resto de la API.
...
- [ ] Pruebas de integración con Spring Boot Test/MockMvc de: fijar, cambiar, retirar y entrada no válida.
- [ ] El contrato TypeScript del frontend se actualiza explícitamente si cambia la forma de la respuesta.
- [ ] `./mvnw verify` deja pruebas y Spotless en verde.

**Depende de:** FS-118.2.
**Nota de riesgo:** las pruebas deben usar el perfil `test` y H2 en memoria, como las pruebas de autenticación existentes. Si apuntan por error a la base persistida de desarrollo, producirán resultados dependientes del estado local.
...
- [ ] `npm run lint`, `npm run build` y Prettier quedan en verde con Node 22.
...
**Depende de:** FS-118.4.
**Nota de riesgo:** es L si además debe crear todos los fixtures reutilizables del nuevo dominio de tareas; la infraestructura JUnit/Spring y la base H2 en memoria ya existen (R-7 del PRD). Si otra historia ya creó esos fixtures, baja a M. Es el último de la cadena y el portador de los criterios negativos, que son la identidad del producto: por eso es el primero que se sacrifica si la historia se alarga, y por eso conviene no dejarlo para el final.
...
graph TD
    ...
    R7["fixtures de tareas<br/>(R-7, por extender)"] -.-> T3
    R7 -.-> T5
```
