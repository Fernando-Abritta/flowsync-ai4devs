...
FlowSync es la respuesta permanentemente disponible a una pregunta concreta —_¿en qué está cada uno ahora mismo, y qué hay libre?_— legible **sin coste social**.
...
En el momento en que actualizar deje de ser algo que hago _para mí_ y pase a ser algo que hago _para que otros vean_, se convierte en reporte, y el reporte se abandona en tres semanas.
...
_Aceptación:_ tras registrarse queda con sesión iniciada y ve el espacio sin ningún paso adicional de invitación o alta en un equipo.

**RF-2 — Inicio y cierre de sesión.** Debe poder iniciar sesión con sus credenciales y cerrarla explícitamente.
_Aceptación:_ con credenciales incorrectas se muestra un error comprensible y no se accede; tras cerrar sesión, volver atrás en el navegador no devuelve el contenido del espacio.

**RF-3 — Persistencia de sesión.** La sesión debe sobrevivir a una recarga de página y al cierre de la pestaña.
_Aceptación:_ recargar no obliga a volver a introducir credenciales.

**RF-4 — Protección del espacio.** Sin sesión iniciada no debe ser posible ver ni modificar ninguna tarea.
_Aceptación:_ acceder a cualquier vista del espacio sin sesión redirige a inicio de sesión; ninguna tarea es visible en ese estado.
...
_Aceptación:_ con el título relleno y ningún otro campo tocado, la tarea se crea, queda asignada a quien la crea y en estado «Pendiente». No se solicita ni sugiere ningún otro dato.
...
_Aceptación:_ intentar crear una tarea sin título, o solo con espacios, se rechaza con un mensaje comprensible junto al campo. El título admite con holgura una frase descriptiva completa; ante uno desmedido el sistema avisa, en lugar de recortarlo en silencio [SUPUESTO].

**RF-7 — Responsable por defecto.** Al crear una tarea, el responsable por defecto es quien la crea.
_Aceptación:_ una tarea recién creada aparece con el nombre de su autor como responsable, sin haberlo seleccionado.
...
_Aceptación:_ no existe en el producto ninguna vía para crear o configurar estados.
...
_Aceptación:_ el cambio se completa en **2 interacciones o menos** desde la lista visible, y el nuevo estado se refleja en la vista inmediatamente.
...
_Aceptación:_ una persona puede asignarse una tarea ajena sin ningún permiso especial ni advertencia.

**RF-11 — Editar una tarea.** Debe poder modificarse el título de cualquier tarea, propia o ajena.
_Aceptación:_ el cambio queda reflejado para todos los miembros del espacio.

**RF-12 — Borrar una tarea.** Debe poder borrarse cualquier tarea, propia o ajena.
_Aceptación:_ la acción pide confirmación explícita, ya que no hay histórico ni papelera y la pérdida es irreversible; tras confirmar, la tarea desaparece para todos.
...
_Aceptación:_ el flujo de creación no contiene el campo fecha ni lo sugiere; la fecha se consulta y se establece únicamente al abrir la tarea; una tarea sin fecha es un estado normal y no genera aviso ni marca de ningún tipo.
...
_Aceptación:_ pasar una tarea vencida a «Hecho» deja de marcarla como vencida sin tocar la fecha; una tarea sin fecha nunca está vencida; una tarea con fecha futura nunca está vencida. «Anterior al día actual» significa el día de quien está mirando [SUPUESTO]: con el equipo de referencia repartido en 3 husos, dos personas pueden ver la misma tarea de forma distinta durante unas horas, y se acepta.
...
_Aceptación:_ la lista no muestra fechas ni marcas de vencimiento; al abrir una tarea vencida, su condición de vencida es evidente sin cálculo mental.

**RF-20 — Filtrar por estado.** Debe poder filtrarse la lista por estado, para centrarse en lo pendiente.
_Aceptación:_ el filtro es la única dimensión de filtrado disponible; no existe filtro por responsable.
...
_Aceptación:_ al marcar una tarea como hecha, desaparece de la vista por defecto; sigue siendo accesible mediante el filtro por estado.
...
_Aceptación:_ dos personas distintas ven el mismo conjunto de tareas; no hay vistas privadas ni contenido oculto por usuario.
...
_Aceptación:_ alguien que abre la aplicación puede enumerar en qué está trabajando cada miembro sin hacer clic en ninguna tarea.
...
_Aceptación:_ con dos personas mirando la lista, un cambio de estado hecho por una es visible para la otra sin que esta haga nada. Este es el requisito que distingue a FlowSync de un tablero compartido cualquiera; si se cae, el producto pierde su razón de ser.
...
_Aceptación:_ con un formulario a medio rellenar o una tarea abierta, la llegada de un cambio ajeno no altera lo que el usuario está haciendo.
...
_Aceptación:_ no existe ningún indicador de conexión, «última vez visto» ni similar en ninguna vista. Requisito negativo deliberado: forma parte de la identidad del producto.

**RF-23 — Estado vacío con salida.** Con el espacio sin tareas, la lista debe explicar qué es y ofrecer crear la primera.
_Aceptación:_ una persona recién registrada puede crear su primera tarea desde la lista vacía sin buscar dónde.
...
**R-1 — Tecnología ya elegida.** El producto se construye sobre Spring Boot 4.1 con Java 21 en el servidor y React 19 en la interfaz. Es una decisión tomada: este MVP no evalúa alternativas ni introduce piezas nuevas.
...
**R-7 — La base de pruebas existe, pero el dominio de tareas no.** El proyecto ya verifica automáticamente la autenticación con JUnit, Spring Boot Test y H2 en memoria. Ningún criterio de aceptación del dominio de tareas se comprueba todavía porque esa capability aún no existe. Cada incremento del MVP debe extender esa base con pruebas unitarias o de integración y mantener `./mvnw verify` en verde.
...
_Para decidirlo hace falta:_ fijar una regla de orden por defecto, decidir si la lista se agrupa por persona y revisar si el filtro por responsable debe volver — o si el tamaño real de uso lo hace innecesario. Conviene decidirlo con una lista poblada delante, no en abstracto.
...
_Para decidirlo hace falta:_ elegir entre no hacer nada, un límite blando con aviso, o destacar una sola tarea como foco actual. Depende de cómo trabaje realmente el equipo, dato del que hoy no disponemos.
...
_Para decidirlo hace falta:_ definir qué observación bastaría para dar el supuesto por bueno o por falso, y resolver si la lente personal vuelve. Ligado a PA-3.
...
_Para decidirlo hace falta:_ reconocer las tres superficies al estimar el trabajo y, opcionalmente, decidir si el selector de personas admite una forma más simple.
...
_Para decidirlo hace falta:_ declarar explícitamente si se puede volver de «Hecho», y si el filtro es recuperación suficiente o hace falta algo más inmediato.
...
_Para decidirlo hace falta:_ elegir el comportamiento visible (avisar, cerrar, quién gana). Es poco trabajo, pero hoy está indefinido.
...
_Para decidirlo hace falta:_ sustituir cada adjetivo por un umbral o un protocolo de comprobación, y dar a P-3 un criterio que pueda fallar de verdad.
...
_Para decidirlo hace falta:_ decidir si el MVP se publica en algún entorno alcanzable. Si no, se queda tal cual y basta con dejarlo escrito.
...
_No hay nada que decidir:_ solo que la estimación deje de contarlo como gratis.
