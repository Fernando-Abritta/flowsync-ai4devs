...
### D4. `dueDate` en los validadores con `vine.date`; la conversión a `DateTime` ya es global

`dueDate: vine.date({ formats: ['YYYY-MM-DD'] }).nullable().optional()` tanto en el validador de creación como en el de actualización, con un builder compartido `calendarDate()` que también usa la cabecera (D3). El proyecto ya registra en `start/validator.ts` un `VineDate.transform` global que devuelve toda fecha validada como `DateTime` de luxon, así que el controlador asigna el valor validado (o `null`) directamente al modelo, sin ninguna conversión propia. En `update`, la comprobación de «cuerpo sin nada que actualizar» pasa a exigir que `status`, `assigneeId` y `dueDate` sean todos `undefined`; `dueDate === null` cuenta como «quitar la fecha», no como ausencia.

Por qué `vine.date` y no `vine.string().regex(...)`: la regex aceptaría `2026-02-30`; `vine.date` con formato único es estricto y devuelve `rule: "date"`, que el cliente traduce. Por qué no convertir en el controlador: el transform global de `start/validator.ts` es el único punto donde un `Date` de VineJS se convierte en `DateTime`, y ya existía antes de este change; un segundo punto de conversión sería código muerto. VineJS crea el `Date` a medianoche en el huso del proceso y `fromJSDate` lo lee en ese mismo huso, así que el día no se desplaza sea cual sea `TZ` (comprobado en la revisión con `UTC`, `America/Argentina/Buenos_Aires`, `Pacific/Kiritimati`, `Pacific/Midway` y `Asia/Kolkata`); la comparación del modelo por `toISODate()` lo blinda además contra cualquier zona. Trade-off: `isOverdue` en el cuerpo no necesita tratamiento: VineJS descarta lo no declarado, igual que hoy `title` en `update`.
...
- [Un `Date` de VineJS y un `DateTime` de luxon pueden desplazar el día si el huso del proceso cambia] → conversión en un único punto (el transform global de `start/validator.ts`), comparación por día en el modelo y `TZ=UTC` fijado en `.env.example`; la batería `curl` incluye una fecha igual al día de referencia para detectar el desplazamiento.
