...
`app/services/reference_day.ts` expone `referenceDayFrom(request): DateTime`: lee la cabecera `X-Client-Date`; si no viene, devuelve el día en curso en UTC; si viene y no es `AAAA-MM-DD` válido (patrón más `DateTime.fromISO(...).isValid`), lanza `E_VALIDATION_ERROR` construido a mano con `rule: "date"` y `field: "clientDate"`, que el manejador de errores convierte en el mismo `422` de cualquier validación. Cada acción del controlador llama al servicio una vez y pasa el día a `TaskTransformer.transform(tasks, referenceDay)`; el transformer lo recibe en su constructor y calcula `isOverdue` con `resource.isOverdueOn(referenceDay)`.
...
`lib/api.ts` calcula el día local del navegador con `getFullYear`/`getMonth`/`getDate` (nunca con `toISOString`, que daría el día en UTC) y lo envía como `X-Client-Date` en todas las llamadas de tareas. `updateTaskStatus` se sustituye por `updateTask(token, id, patch)` con `patch: { status? , dueDate? }` (tipo `UpdateTaskPayload`), y se añade `getTask(token, id)`. El tipo `Task` gana `dueDate: string | null` e `isOverdue: boolean`. La traducción de errores añade `dueDate` al mapa de etiquetas y la regla `date` («Introduce una fecha válida.»); `clientDate` no se traduce porque no puede ocurrir desde el frontend y caerá en el mensaje genérico.
